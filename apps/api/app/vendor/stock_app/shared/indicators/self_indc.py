...
def _vwap_bands(work: pd.DataFrame, freq: str) -> pd.DataFrame:
    ...
    grouper = _vwap_group_keys(work.index, freq)
    tpv = typical * volume
...
def _vwap_group_keys(index, freq: str):
    if isinstance(index, pd.DatetimeIndex):
        normalized = index.normalize()
        if freq == "D":
            return normalized
        week_offsets = pd.to_timedelta(index.weekday, unit="D")
        return normalized - week_offsets
    if freq == "D":
        return [value.date() if hasattr(value, "date") else value for value in index]
    return [str(value)[:10] for value in index]

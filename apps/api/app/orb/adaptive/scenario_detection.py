...
def detect(features: dict, errors: tuple[str, ...], capabilities: tuple[Capability, ...], now: int) -> dict[str, str]:
    ...
    invalid = bool(errors)
    risk_context_verified = "RISK_CONTEXT" in names

    def ext(name: str, *, risk_only: bool = False) -> str:
        if name in names:
            return "RISK_ARMED_EXTERNAL" if risk_only else "OBSERVED_EXTERNAL_FLAG"
        return "NOT_OBSERVED_VERIFIED_CONTEXT" if risk_context_verified else "UNOBSERVABLE_EXTERNAL_INPUT"
    ...
    elif risk_context_verified or "VIX" in names:
    ...
    if "RESULT_DAY" in names:
        ...
        out["C01"] = "NOT_OBSERVED_VERIFIED_CONTEXT" if risk_context_verified else "UNOBSERVABLE_EXTERNAL_INPUT"
    ...
    if "EX_DIVIDEND_UNADJUSTED" in names:
        ...
        out["C04"] = "NOT_OBSERVED_VERIFIED_CONTEXT" if risk_context_verified else "UNOBSERVABLE_EXTERNAL_INPUT"
    ...
    if {"ORDER_REJECTED", "BROKER_SQUAREOFF_RISK"} & names:
        ...
        out["E02"] = "NOT_OBSERVED_VERIFIED_CONTEXT" if risk_context_verified else "UNOBSERVABLE_EXECUTION_INPUT"
    ...
    if "BACKTEST_LIVE_MISMATCH" in names:
        out["E04"] = "OBSERVED_EXTERNAL_FLAG"
    else:
        out["E04"] = "NOT_OBSERVED_VERIFIED_CONTEXT" if risk_context_verified else "UNOBSERVABLE_REQUIRES_REPLAY_LIVE_COMPARISON"

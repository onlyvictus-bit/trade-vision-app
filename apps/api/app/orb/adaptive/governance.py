...
_FINGERPRINT_SUFFIXES = frozenset({".py", ".json", ".toml", ".yaml", ".yml"})


def _fingerprint_manifest(root: Path) -> dict[str, str]:
    """Hash every decision-relevant source/config file under *root* recursively.

    Relative paths are part of the manifest so future subpackages and duplicate
    filenames cannot silently collapse onto the same key. Generated bytecode,
    caches, logs and unrelated text files are intentionally excluded.
    """
    files = sorted(
        (
            path
...
def code_fingerprint_manifest() -> dict[str, str]:
    """Return the deterministic adaptive-engine source/config hash manifest."""
    return _fingerprint_manifest(Path(__file__).resolve().parent)


def code_fingerprint() -> str:
    return digest(code_fingerprint_manifest())
